...
class Test(unittest.TestCase):
    ...
    def test(self):
        ...
        root.save()
        project1_analytic_acc = AnalyticAccount(root=root,
                                                parent=root,
                                                name='Project 1')
        project1_analytic_acc.save()
        project2_analytic_acc = AnalyticAccount(root=root,
                                                parent=root,
                                                name='Project 2')
        project2_analytic_acc.save()
        project3_analytic_acc = AnalyticAccount(root=root,
                                                parent=root,
                                                name='Project 3')
        ...
        expense.reload()
        self.assertEqual(expense.debit, Decimal('3500.00'))
        payable.reload()
        self.assertEqual(payable.credit, Decimal('3500.00'))

        # Check analytic accounts amounts
        project1_analytic_acc.reload()
        self.assertEqual(project1_analytic_acc.debit, Decimal('2000.00'))
        project2_analytic_acc.reload()
        self.assertEqual(project2_analytic_acc.debit, Decimal('1500.00'))
        ...
        self.assertEqual([l.analytic_lines for l in move2.lines],
                         [[], [], [], []])
        self.assertEqual(
            sorted([
                l.analytic_accounts[0].account.name for l in move.lines
                if l.account.id == expense.id
            ]), ['Project 1', 'Project 2'])

        # Post the duplicated move and check analytic accounts amounts
        move2.click('post')
        project1_analytic_acc.reload()
        self.assertEqual(project1_analytic_acc.debit, Decimal('4000.00'))
        project2_analytic_acc.reload()
        self.assertEqual(project2_analytic_acc.debit, Decimal('3000.00'))
        ...
        self.assertEqual(analytic_account2.account.id, 2)
